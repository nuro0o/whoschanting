...
Tune the player limits, cultist counts by room size, phase lengths, missions, and ritual goals in `config/game.php`. Explicit `ritual_goals_by_player_count` entries override the multiplier. Bump `rules_version` when adjusting balance so statistics remain comparable. Starting a match snapshots its rules version, player count, phase lengths, mission, and goal; configuration changes apply to the next gathering. New matches use `final-ritual-vote-v1`. The final-vote rule and two-player ending also apply to ongoing matches; finished matches remain finished. Self-veiling is not enabled.
