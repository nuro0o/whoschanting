...
The server replaces toxic player names with a random funny village name, such as `Wobbly Hollow` or `Pickle Crossing`, from 4,096 combinations in `app/Game/VillageNames.php`. Replacements avoid names already at the table and remain consistent across reconnects, chat attribution, and new match recaps. Existing toxic seats receive a stable replacement when loaded, which is saved when the player accesses or updates the room. Previously archived matches are not rewritten. Chat, role claims, discussion answers, Last Words, and private match feedback still replace listed profanity with `*`.

The English starter word list lives in `config/moderation.php`. Matching handles capitalization, common number/symbol substitutions, repeated letters, fullwidth letters, and separators inside words. Chat uses whole-word boundaries. Player names also check the configured `name_fragments` to catch compounds such as `shithead` and decorated names such as `xxSh1thead99`, while preserving ordinary names such as Scunthorpe and assistant. Add explicit inflections or additional languages to the list as needed; this is a word filter, not contextual abuse detection or a complete moderation system for public rooms.
...
New matches use `large-gatherings-v1`; matches already in progress retain their existing flow. The duration is snapshotted from `config/game.php` at match start. No database migration is needed. Rebuild the frontend and restart long-running application processes when deploying.
...
**The Fae Court** is the first paid faction expansion. One owner at the table unlocks a randomly dealt Fae Broker for everyone in a Classic or Illusions room of 7–15 players. The Broker offers anonymous night bargains and can share a Town or Cult victory. Enable it in lobby mode settings; see [Fae Court rules and checkout setup](docs/fae-court.md).

The **Profile store** at `/settings/profile#store` lets verified players spend virtual **Crowns** on permanent cosmetic titles, accents, and portrait backdrops. Qualifying completed matches award 1 Crown per starting player, plus 5 for winning with 1–6 players or 10 with 7+ players. Shared Wi-Fi is welcome in public and private rooms. Each verified account receives at most one reward per match; repeated very short matches trigger a temporary crown-earning cooldown without stopping play, XP, or achievements. Purchases become available in the Progression wardrobe; cosmetics never change gameplay. Three optional one-time Stripe bundles add portrait cosmetics, host table variations, 3D banishment effects and victory celebrations chosen once from a random winning player. Configure the Stripe key, dedicated webhook secret and three Price IDs to enable checkout. Run `php artisan migrate` and rebuild the frontend. See [cosmetic store and Crowns](docs/cosmetic-store.md) for earning rules, payment setup, refunds and shared-account compatibility with Geniousverse.
...
**The Looking Glass** in the wardrobe creates an original villager from layered illustrations. Body Type 1 has male faces and builds; Body Type 2 has female faces and builds. Both support Classic, Three-quarter, and Defiant poses with matching artwork. Choose among four face archetypes, five hairstyles (or bald), five hats (or no hat), four outfits, and three accessories (or none), with independent complexion, hair, and clothing colors. Changing body type or pose keeps the other selections. Arrow controls preview each part, including locked items; Randomize chooses only unlocked parts. Save customization equips the creation, while Revert restores the saved look. The captain's tricorn and moth brooch unlock at level 2; the antler crown and moonlit mantle unlock at level 3. Both body types, all three poses, and all other creation options are available to verified accounts immediately.

One versioned creation is stored with the account's existing customization JSON. Selecting an original villager preserves the saved creation. A saved creation appears in room and lobby character pickers, and its parts, body type, and pose are copied into the room when worn; subsequent wardrobe edits do not alter an active match. Older recipes without body type or pose use Body Type 1 and Classic. Guests continue to receive an original villager. The server rejects unknown, incomplete, unsupported, or locked part combinations; uploaded images, arbitrary colors, and external asset URLs are not accepted. Definitions and unlock levels live in `config/character_creator.php`. Deploy the PNG atlases under `public/assets/chanting/creator/` and rebuild the frontend; no database migration or `.env` change is needed. See [creator artwork and exact generation prompts](docs/character-creator-art.md).
...
- **3–15 players**: 3–4 players have **1 cultist**, 5–6 have **2**, 7–8 have **3**, 9–12 have **4**, and 13–15 have **5**. In the Classic roster, exactly one cultist is the Veilweaver. A Dreamweaver replaces one Acolyte at 7+ players; remaining cultists are Acolytes. There is always one Oracle. A Warden joins at 5+ players, a Lamplighter at 7+, a Medium at 8+, and a Bellkeeper at 10, each replacing a Townsperson; remaining town seats stay Townspeople. Cultists know all their teammates. In 5–15-player matches they receive the same randomly selected mission. **Small Gathering (3–4 players)** gives the lone Veilweaver **A voice below**: each submitted chant advances the ritual by one step, even when investigated. The lobby previews the current roster and ritual goal before players ready up.
...
For up to 10 players, discussion lasts **90 seconds below one-third ritual progress, 70 seconds from one-third, and 50 seconds from two-thirds onward**, including the final discussion. Larger groups add 10 seconds per player above 10 to each tier. Each dawn announces the available time. Unanimous readiness can still end discussion early; voting keeps its usual duration. Tune `paranoia_discussion_seconds` in `config/game.php`; the role pool, team counts and discussion timings are snapshotted at match start. Rematches keep the mode and draw a new cast. New matches use rules version `large-gatherings-v1`. No migration or `.env` change is needed.
...
- **Classic:** the 3–15-player setup is the default, with the existing Illusions variant available from 5 players.
