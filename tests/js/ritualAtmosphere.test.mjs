...
await test('ten-step ritual stays quiet after the first task and advances at each 20% milestone', () => {
...
await test('short rituals skip intermediate artwork in proportion to completion', () => {
    assert.deepEqual(
...
await test('artwork changes at the milestone, never before, with the final scene reserved for completion', () => {
...
await test('empty and malformed game state stays quiet and valid progress is clamped', () => {
...
await test('normal dawn clears the night, but the final ritual keeps night through discussion and voting', () => {
...
await test('lobby, role reveal, and the finished game reset the atmosphere even if the final-vote flag remains set', () => {
